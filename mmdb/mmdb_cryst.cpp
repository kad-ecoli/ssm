...
int      i,j,k;
 ...
 if (areMatrices())  { // if orth2frac and frac2orth matrices are defined

    // U is in upper triangular form
    Tmp[0][0] = U[0];
    Tmp[1][1] = U[1];
    Tmp[2][2] = U[2];
    Tmp[0][1] = U[3];
    ...
    // ROU is a 4x4 matrix, but we only use the upper 3x3 submatrix
    for (i=0;i<3;i++)
      for (j=0;j<3;j++)  {
        A [j][i] = ROU[j][i];
        AT[i][j] = ROU[j][i];
